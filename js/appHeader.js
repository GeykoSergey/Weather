...
export const createHeader = (city) => {
    ...
    cityChange.textContent = 'Змінити місто';
    cityLocation.textContent = 'Моє місцезнаходження';
    ...
    unitsC.addEventListener('click', () => {
        ...
        document.querySelector('.weather__units').textContent = '0';
    ...
    window.addEventListener('click', (event) => {
        if (event.target == searchInput || event.target == searchBtn || event.target == cityChange) {
            return;
        } else {
            headerCity.innerHTML = '';
